...
- **Serialization blobs:** `typeInstance` and `outputTypeInstance` store binary property-list payloads describing Swift types and default values. Pair them with `typeId` joins to `Types`/`ToolParameterTypes` to avoid having to deserialize when only the high-level type is needed.
